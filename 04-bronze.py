...
class Bronze():
    ...
    def consume_kafka_multiplexed(self,once=True, processing_time="5 seconds"):
        import pyspark.sql.functions as F
    ...
    def consume(self, once=True, processing_time = "5 seconds"):
        ...
        self.consume_kafka_multiplexed(once, processing_time)
    ...
    def assert_count(self, table_name, expected_count, filter = "true"):
        ...
        actual_count = spark.read.table(f"{self.catalog}.{self.db_name}.{table_name}").filter(filter).count()
